...
async function openGame(game){
  if(!['2K18','2K19','2K20','2K22','2K23','2K24','2K25','2K26'].includes(game)) return;
...
function home(){
 return `<main class="shell">${header('Definitely not OTG! with suplexes.')}<section class="hero"><h2>Choose a game</h2><p>One simple engine. Eleven WWE 2K datasets. 2K18, 2K19, 2K20 and 2K22 through 2K26 are live; the older games will plug into the same structure as their data is built.</p></section><div class="grid">${GAMES.map(g=>{const ready=['2K18','2K19','2K20','2K22','2K23','2K24','2K25','2K26'].includes(g);const label=g==='2K26'?'OPEN &bull; SDH 501':g==='2K25'?'OPEN &bull; SDH 436':g==='2K24'?'OPEN &bull; SDH 338':g==='2K23'?'OPEN &bull; SDH 250':g==='2K22'?'OPEN &bull; SDH 228':g==='2K20'?'OPEN &bull; SDH 266':g==='2K19'?'OPEN &bull; SDH 252':g==='2K18'?'OPEN &bull; SDH 220':'DATA QUEUED';return `<button class="game-tile ${ready?'ready':''}" ${ready?'':'disabled'} data-game="${g}"><strong>WWE ${g}</strong><small>${label}</small></button>`}).join('')}</div><div class="footer">Local progress and custom teams are stored on this device &bull; v${CURRENT_VERSION}.</div></main>`
}
