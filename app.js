...
function header(sub=''){
 return `<div class="topbar">${state.view==='game'?'<button class="back" data-action="home">← Games</button>':''}<div class="brand"><h1>WWE Title Universe</h1><p>${esc(sub)}</p></div><div class="version">v0.1.0</div></div>`
}
function home(){
...
function game(){let content='';if(state.section==='dashboard')content=dashboard();if(state.section==='roster')content=roster();if(state.section==='championships')content=genericUnlock('championships',state.data.championships,'name',x=>`${x.category} • ${x.competition} • ${x.division}`);if(state.section==='arenas')content=genericUnlock('arenas',state.data.arenas,'name',x=>`${x.category} • ${x.unlockNote||''}`);if(state.section==='tag teams')content=tagTeams();if(state.section==='tournaments')content=tournaments();return `<main class="shell">${header(`WWE ${state.game} • ${state.data.status}`)}${nav()}${content}<div class="footer">Source policy: SmackDown Hotel preferred. This build is the engine test, not the final 2K26 dataset.</div></main>`}
